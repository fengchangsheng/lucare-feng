...
//        sched.scheduleJob(trigger);
        ...
        List<? extends Trigger> jobTriggers = sched.getTriggersOfJob(jobKey("myJob", "group1"));
        for (Trigger trigger1 : jobTriggers) {
            System.out.println(trigger1.getKey().getName());
            sched.unscheduleJob(trigger1.getKey());
        }

        SimpleTrigger trigger3 = (SimpleTrigger) newTrigger().withIdentity("trigger3", "group1")
                .forJob(jobKey("myJob", "group1"))
                .startAt(sdf.parse("2016-10-23 20:06:20"))
                .build();

        SimpleTrigger trigger4 = (SimpleTrigger) newTrigger().withIdentity("trigger4", "group1")
                .forJob(jobKey("myJob", "group1"))
//                .forJob("myJob")这是错误关键   没有指定组
                .startAt(sdf.parse("2016-10-23 20:06:25"))
                .build();
    ...
    public void method2() throws SchedulerException, ParseException {
        ...
        SimpleTrigger trigger = (SimpleTrigger) newTrigger().withIdentity("trigger1", "group1")
                .startAt(sdf.parse("2016-10-23 15:41:30"))
                .build();

        SimpleTrigger trigger2 = (SimpleTrigger) newTrigger().withIdentity("trigger2", "group1")
                .startAt(sdf.parse("2016-10-23 15:41:38"))
                .build();
